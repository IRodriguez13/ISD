...
PKG="${ROOT}/packages/libxau"
PREFIX="${PKG}/prefix/${ARCH}"
PROTO="${ROOT}/packages/xorgproto/prefix/${ARCH}/usr"
[ -d "${PKG}/src" ] || { echo "✗ missing libXau source; run: make fetch PROFILE=desktop" >&2; exit 1; }
mkdir -p "$PREFIX"
cd "${PKG}/src"
...
test -f "$PREFIX/usr/lib/libXau.a"
echo "✓ libXau OK → $PREFIX"
